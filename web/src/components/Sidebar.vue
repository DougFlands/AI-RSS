<template>
  <div class="h-full flex flex-col">
    ...
    <div class="flex-grow overflow-y-auto p-3">
      ...
      <el-empty description="没有订阅源，请添加" v-else-if="sources.length === 0"></el-empty>
    ...
    >
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button type="danger" v-if="isEditing" @click="confirmDeleteSource" class="delete-btn">删 除</el-button>
          <el-button type="primary" @click="saveSource" :loading="isAdding" class="confirm-btn">确 定</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
...
const saveSource = async () => {
  ...
  try {
    ...
    errorMessage.value = error.response?.data?.error || (isEditing.value ? '更新订阅源失败' : '添加订阅源失败');
